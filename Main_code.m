clear variables, close all, clc              % clear varibles etc.
%% General model geometry and resolution
L            = 400;                          % model height/ sample length
nz           = 601;                          % number of grid points
dz           = L/(nz-1);                     % grid spacing (m)
Z            = -L/2:dz:L/2;                  % coordinate
...
%    b_W          = 0.05;                      % factor to change DW based on W
...
A1           = 1/(2*eta1);                   % preexponential factor for strong
...
eta_Huet     = ones(1,length(W));            % Initializationfor visocsity array
...
Time         = [0];                                     % Time array
time         = 0;                                       % initial time
tolerance    = 1e-6;                                    % error for iterative solver
...
%             W(I)         = W(I) + dt_pt*Res_W(I);                                                              % iterative update W
%             W(I_C)       = 1;                                                                                  % keep initial W in center
...
%         H(I_H)          = 1;                                                                                  % keep initial H in center                                                        
   ...
   if mod(ti,100)==1                                                         % save/show every 25th time step
        figure(1)
        subplot(241), hold off
        plot(Z,Vxc,'-k'), hold on, plot(Z,Vx0,'--k'), xlabel('Z'), ylabel('Vx')
        ...
        plot(Z,H,'-r')
        xlabel('Z'), ylabel('W & H'), axis([-L/2 L/2 0 1]), legend('W','initial W','H bulk')
        ...
        plot(Z,eta_Huet_H,'-r')                                                     
        xlabel('Z'), ylabel('Viscosity'), axis([-L/2 L/2 0 1])
        set(gca,'yscale','log')
        grid on
        legend('\eta mix total','\eta mix strong')
               
        % plot velocity
        subplot(244), hold off                        
        plot(Z,Strain_prof)                                                     
        title('Strain profile log10')
        ylabel('strain incremental'),xlabel('Z')
        
        % plot shear zone thickness
        subplot(245)
        plot(Time(2:end),SZthickness,'-r')
        legend('SZT (meanStrain)','location','northwest')
        xlabel(['Time step: ',num2str(ti)]), ylabel('shear zone thickness')
        
        % gamma plotten
        subplot(246)
        plot(Time(2:end),Gamma,'b-')
        legend('Gamma','location','northwest')
        xlabel(['Time step: ',num2str(ti)]), ylabel('gamma')
        
        % displacement plotten
        subplot(247), hold off
        plot(Z,Dis_prof_tot/(SZthickness(end)),'k-')
        ylabel('normalized Disp.'),xlabel('Z')
        
        subplot(248)
        plot(Z,Strain_prof,'-k'), hold on
        plot([Z(1) Z(end)],[mean(Strain_prof) mean(Strain_prof)],'-r'), hold off
        ylabel('Accumulated strain profile'),xlabel('Z')
...
save Data_PLOT_W_nl Gamma_PLOT_W_nl Dis_PLOT_W_nl Visc_PLOT_W_nl Z;           % add: DW_var_PLOT_nl if DW is variable
...
axis([0 L/2 0 1.0])
xlabel('Z/(SZT/2)'),ylabel('C')                                             % normalized x-axis
...
plot(Z/(SZthickness(end)/2),(Dis_prof_tot-mean(Dis_prof_tot))/SZthickness(end),'-*r'), hold on          
hei         = max([max(Strain_prof) max(Dis_prof_tot-mean(Dis_prof_tot))]); 
xlim([0 5]);
title(['SZ thickness: ',num2str(max(SZthickness))])
xlabel('Z')
legend('Strain','Displacement','SZ width/2')
grid on
drawnow
